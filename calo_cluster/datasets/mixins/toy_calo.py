...
class ToyCaloDataModuleMixin(PandasDataModule):
    ...
    def prepare_data(self) -> None:
        if not self.data_exists():
            logging.info(
                f'dataset not found at {self.transformed_data_dir}.')
            self.make_data()

    @staticmethod
    def _generate_events(n_events, nc_mu: float = 10.0, np_mu: float = 1.0, p_s: float = 5.0):
        evts = []
    ...
    @staticmethod
    def generate(data_dir, n_events=10000, nc_mu: float = 10.0, np_mu: float = 1.0, p_s: float = 5.0) -> None:
        logging.info(f'Generating data at {data_dir}.')
        events = ToyCaloDataModuleMixin._generate_events(n_events, nc_mu, np_mu, p_s)
        for i in tqdm(range(n_events)):
            df = events[i]
    ...
    def make_data(self):
        self.generate(self.data_dir, nc_mu=self.nc_mu, np_mu=self.np_mu, p_s=self.p_s)
