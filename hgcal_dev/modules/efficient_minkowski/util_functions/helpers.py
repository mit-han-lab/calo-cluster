...
def sparse_quantize(coords,
    ...
    if use_label:
        _, inds, invs, counts = np.unique(key, return_index=True, return_inverse=True, return_counts=True)
...
def sparse_collate(coords, feats, labels=None, is_double=False, coord_float=False):
    ...
    for coord, feat in zip(coords, feats):
        ...
        if not coord_float:
            ...
            coords_batch.append(
                torch.cat((coord, torch.ones(num_points, 1).float() * batch_id), 1))
