...
__all__ = [
    'Config', 'configs', 'update_configs_from_module',
    'update_configs_from_arguments'
]


class Config(G):
    def __init__(self, func=None, args=None, keys=None, detach=False,
                 **kwargs):
        super().__init__(**kwargs)

        if func is not None and not callable(func):
            raise TypeError(
                'func "{}" is not a callable function or class'.format(
                    repr(func)))
        if args is not None and not isinstance(
                args, (collections.Sequence, collections.UserList)):
            raise TypeError(
                'args "{}" is not an iterable tuple or list'.format(
                    repr(args)))
        if keys is not None and not isinstance(
                keys, (collections.Sequence, collections.UserList)):
            raise TypeError(
                'keys "{}" is not an iterable tuple or list'.format(
                    repr(keys)))
    ...
    def __call__(self, *args, **kwargs):
        ...
        while queue:
            ...
            if isinstance(x, (collections.Sequence, collections.UserList
...
def update_configs_from_module(*mods):
    ...
    def exec_module_once(module):
        ...
        spec = importlib.util.spec_from_file_location(os.path.basename(module),
                                                      module)
...
def update_configs_from_arguments(args):
    ...
    while index < len(args):
        ...
        if '=' in arg:
            index, keys, val = index + 1, arg[:arg.index('=')].split(
                '.'), arg[arg.index('=') + 1:]
        else:
