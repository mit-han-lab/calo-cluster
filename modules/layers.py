...
class ConvolutionBlock(nn.Module):
            ...
            OrderedDict([
                ('conv',
                 MinkowskiConvolution(inc,
                                      ...
                                      stride=stride,
                                      transpose=transpose)),
                ('bn', MinkowskiBatchNorm(outc)),
                ('act',
                 MinkowskiReLU(True) if not self.no_relu else nn.Sequential())
            ]))
        self.init_weights()
    ...
    def load_weights(self, nas_module, runtime_inc_constraint=None):
        ...
        self.net.bn.bn.weight.data = nas_module.net.bn.bn.weight[:self.outc]
        self.net.bn.bn.running_var.data = nas_module.net.bn.bn.running_var[:
                                                                           self
                                                                           .
                                                                           outc]
        self.net.bn.bn.running_mean.data = nas_module.net.bn.bn.running_mean[:
                                                                             self
                                                                             .
                                                                             outc]
        self.net.bn.bn.bias.data = nas_module.net.bn.bn.bias[:self.outc]
        self.net.bn.bn.num_batches_tracked.data = nas_module.net.bn.bn.num_batches_tracked
...
class DynamicConvolutionBlock(RandomModule):
    def __init__(self,
        ...
        self.net = nn.Sequential(
            OrderedDict([
                ('conv',
                 DynamicMinkowskiConvolution(inc,
                                             outc,
                                             kernel_size=ks,
                                             dilation=dilation,
                                             stride=stride)),
                ('bn', DynamicMinkowskiBatchNorm(outc)),
                ('act',
                 MinkowskiReLU(True) if not self.no_relu else nn.Sequential())
            ]))
        self.runtime_inc = None
