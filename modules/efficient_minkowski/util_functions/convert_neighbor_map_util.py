...
load_list = [os.path.join(pd, '../others', 'convert_neighbor_map.cpp'), os.path.join(pd, '../others', 'convert_neighbor_map.cu')]
convert = load(
...
class ConvertNeighborMap(Function):
    ...
    def forward(ctx, neighbor_map):
        ...
        map_converted = convert.convert_map_forward(neighbor_map.int(), idx_batch.int(), idx_point.int())
